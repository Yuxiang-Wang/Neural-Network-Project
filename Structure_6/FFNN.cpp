...
//constructor #1
template<typename T, typename fun0, typename fun1, typename fun2>
FFNN<T, fun0, fun1, fun2>::FFNN(const std::vector<FF_NeuronLayer<T>>& in_layers, const fun0& in_activ, const fun1& in_loss, const fun2& in_grad, const T& in_lrn_rate) : layers(in_layers), layer_no(in_layers.size()), nodes_per_layer(in_layers.size()), inputs_per_layer(in_layers.size()), activ(in_activ), loss(in_loss), grad(in_grad), lrn_rate(in_lrn_rate)
{
  //fill nodes_per_layer
  for (int i = 0; i < layer_no; ++i)
    {
...
template<typename T, typename fun0, typename fun1, typename fun2>
template<typename Derived>
Matrix<T, Dynamic, Dynamic> FFNN<T, fun0, fun1, fun2>::query(const MatrixBase<Derived>& in_arg)
{
...
template<typename T, typename fun0, typename fun1, typename fun2>
template<typename Derived>
void FFNN<T, fun0, fun1, fun2>::train(const MatrixBase<Derived>& input_data, const MatrixBase<Derived>& output_data, const int& max_iterations) 
{
  for (int i0 = 0; i0 < max_iterations; ++i0)
  {
    for (int i = 0; i < input_data.template cols(); ++i)
      {
	...
	generate_errors(local_x, local_y);
	...
	for (int i1 = 1; i1 < (layer_no - 1); ++i1)
	  {
	    layers[layer_no - 1 - i1].update_weights(lrn_rate * grad(layers[layer_no - i1].errors, layers[layer_no - i1].outputs, layers[layer_no - 1 - i1].outputs));
	  }
    ...
    std::cout << "Iteration #:" << i0 << std::endl;
      }
}
