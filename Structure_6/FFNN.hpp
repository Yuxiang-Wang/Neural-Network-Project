...
private:
  ...
  //const Matrix<T, Dynamic, Dynamic>& input_data;

  //const Matrix<T, Dynamic, Dynamic>& output_data;
...
public:
  ...
  FFNN(const std::vector<FF_NeuronLayer<T>>& in_layers, const fun0& in_activ, const fun1& in_loss, const fun2& in_grad, const T& in_lrn_rate); 
  ...
  //non-recursive style = linear complexity = O(n)
  template<typename Derived>
  Matrix<T, Dynamic, Dynamic> query(const MatrixBase<Derived>& in_arg);
  ...
  //should this be const?
  //void train(const int& max_iterations = 100);

  //should this be const?
  template<typename Derived>
  void train(const MatrixBase<Derived>& input_data, const MatrixBase<Derived>& output_data, const int& max_iterations = 3);
