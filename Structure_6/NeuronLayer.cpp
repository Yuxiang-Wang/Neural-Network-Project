...
template<typename T>
template<typename Derived>
NeuronLayer<T>::NeuronLayer(const MatrixBase<Derived>& in_weights, const int& in_BiasFlag) : nodes(in_weights.template rows()), inputs((in_weights.template cols()) - in_BiasFlag), BiasFlag(in_BiasFlag), weights(in_weights), outputs(nodes, 1)
{
...
const Matrix<T, Dynamic, Dynamic>& NeuronLayer<T>::operator () (const MatrixBase<Derived>& in_vector, const fun0& in_activ)
{
  ...
  if (BiasFlag)
      ...
      for (int i = 0; i < inputs; ++i)
	{
      ...
      outputs = (weights * local).unaryExpr(in_activ);
  ...
  outputs = (weights * in_vector).unaryExpr(in_activ);
